  ...
  return (
    <div className="grid gap-2">
      {messages.map((message) => (
        <div key={message.id} className="rounded-[18px] border border-white/8 bg-[rgba(7,11,19,0.72)] px-3 py-2.5">
          <p className="truncate text-sm font-medium text-white">{message.subject || "Subject unavailable"}</p>
...
export function ManualUnsubscribeWizard({
  ...
  const complete = totalCount > 0 && resolvedCount >= totalCount;
  ...
  function goToIndex(nextIndex) {
    if (nextIndex < 0 || nextIndex >= totalCount || nextIndex === index) {
      return;
    }
  ...
  return (
    <AnimatePresence>
      ...
      <motion.section
        ...
        aria-modal="true"
        className="fixed left-1/2 top-1/2 z-50 flex max-h-[88vh] w-[min(760px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col rounded-[28px] border border-white/12 bg-[rgba(7,11,19,0.98)] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.38)]"
        ...
        <div className="mt-5 min-h-0 flex-1 overflow-y-auto pr-1">
          {complete ? (
            ...
            <div className="space-y-4">
              <div>
                <p className="text-lg font-semibold text-white">{getGroupTitle(currentGroup)}</p>
                <p className="mt-1 text-sm text-slate-400">{currentGroup.representativeAddress || "Representative address unavailable"}</p>
              </div>
              ...
              {showingPreviouslyHandledPrompt ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    {Number.isFinite(currentGroup.messageCount) ? (
                      <p className="mt-2 text-xs text-slate-500">
                        {`${formatCount(currentGroup.messageCount)} messages in this group`}
                        {Number.isFinite(currentGroup.unreadCount) ? ` · ${formatCount(currentGroup.unreadCount)} unread` : ""}
                      </p>
                    ) : null}
                  </div>
                  ...
                  <div className="grid gap-3">
                    {operations.length > 0 ? operations.map((operation) => (
                      <div key={operation.id} className="rounded-[20px] border border-white/10 bg-white/5 p-3">
        ...
        {!complete ? (
          <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
            ...
            {!showingPreviouslyHandledPrompt ? (
              <button
                className={classNames(
                  "rounded-2xl border px-4 py-2.5 text-sm font-semibold transition-colors duration-200",
                  currentHandled
                    ? "cursor-not-allowed border-white/8 bg-black/18 text-slate-500"
                    : "border-[#f4c95d]/40 bg-[#f4c95d] text-slate-950 hover:border-[#f7d77d] hover:bg-[#f7d77d]",
                )}
                disabled={currentHandled}
                onClick={markCurrentHandled}
                type="button"
              >
                {currentHandled ? "Handled" : "Mark as handled"}
              </button>
            ) : null}
            <button
              className={classNames(
