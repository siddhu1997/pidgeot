...
export function sanitizeSenderGroup(group) {
  ...
  const unsubscribeResolution = resolveUnsubscribeMechanisms({
...
export function listSanitizedSenderGroups(state) {
  ...
  return state.groupOrder
    ...
    .sort((left, right) => {
      ...
      const leftKey = `${left.representativeDomain || ""}:${left.representativeAddress || ""}:${left.id}`;
      const rightKey = `${right.representativeDomain || ""}:${right.representativeAddress || ""}:${right.id}`;
      return leftKey.localeCompare(rightKey);
    });
}
