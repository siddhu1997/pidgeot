...
export function createWorkflowSimulationService({
  ...
  function getBlockedExecution(sessionId, leaseId) {
    ...
    return {
      ...
      cleanupStatus: CLEANUP_EXECUTION_RESULT_STATUSES.PAUSED,
      unsubscribeStatus: UNSUBSCRIBE_EXECUTION_RESULT_STATUSES.PAUSED,
    };
  }
  ...
  async function simulateCleanupAction({ cleanupEligibleMessageIds, leaseId, requestedAt, senderGroupId, sessionId }) {
    if (typeof processingLeaseStore.ensureActiveLeaseForUserAction === "function") {
      processingLeaseStore.ensureActiveLeaseForUserAction({ leaseId, sessionId });
    }
