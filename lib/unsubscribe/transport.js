...
function getPinnedRequestDiagnostic(resolvedTarget) {
  return {
    hostname: resolvedTarget?.hostname || null,
    port: resolvedTarget?.port || null,
    protocol: "https:",
    selectedAddress: resolvedTarget?.selectedAddress?.address || null,
    selectedFamily: resolvedTarget?.selectedAddress?.family || null,
  };
}

function logUnsubscribeTransportDiagnostic(details) {
  console.info("[pidgeot:unsubscribe-transport]", details);
}
...
export function defaultSendPinnedHttpsRequest({
  ...
  return new Promise((resolve, reject) => {
    ...
    let completed = false;
    let settled = false;
    const pin = getPinnedRequestDiagnostic(resolvedTarget);
    ...
    if (signal?.aborted) {
      logUnsubscribeTransportDiagnostic({
        event: "timeout",
        hostname: pin.hostname,
        selectedAddress: pin.selectedAddress,
        selectedFamily: pin.selectedFamily,
      });
    ...
    const request = https.request({
      ...
      lookup(hostname, options, callback) {
        logUnsubscribeTransportDiagnostic({
          ...
          hostname: pin.hostname,
          lookupAll: Boolean(options?.all),
          lookupFamily: options?.family ?? null,
        ...
        const lookupAddress = resolvedTarget.selectedAddress.address;
        const lookupFamily = resolvedTarget.selectedAddress.family;
        const lookupAll = Boolean(options?.all);

        logUnsubscribeTransportDiagnostic({
          event: "lookup_callback",
          hostname: pin.hostname,
          lookupAddress,
          lookupFamily,
          lookupAll,
          lookupResultShape: lookupAll ? "all" : "classic",
          selectedAddress: pin.selectedAddress,
          selectedFamily: pin.selectedFamily,
        });
        ...
        callback(null, lookupAddress, lookupFamily);
      },
      method,
