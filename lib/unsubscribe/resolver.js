...
export function resolveUnsubscribeMechanisms({ observations = [] } = {}) {
  ...
  for (const observation of observations) {
    ...
    for (const target of targets) {
      ...
      summariesByKey.set(`${resolved.summary.type}:${resolved.summary.status}`, resolved.summary);
    }
  }

  const operations = [...operationsById.values()].sort((left, right) => left.id.localeCompare(right.id));
  const mechanisms = [...summariesByKey.values()].sort((left, right) => {
    const leftKey = `${left.type}:${left.status}`;
    const rightKey = `${right.type}:${right.status}`;
