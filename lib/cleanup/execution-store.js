...
function isStickyFailureStatus(status) {
  return (
    status === CLEANUP_EXECUTION_RESULT_STATUSES.FAILED_PERMANENT
    || status === CLEANUP_EXECUTION_RESULT_STATUSES.NO_LONGER_ACTIONABLE
  );
}
...
export function createCleanupExecutionStore({ now = () => Date.now() } = {}) {
  ...
  function clearSession(sessionId) {
    ...
    for (const key of resultsBySessionMessageKey.keys()) {
      if (key.startsWith(sessionPrefix)) {
        resultsBySessionMessageKey.delete(key);
      }
    }

    for (const key of inFlightBySessionMessageKey.keys()) {
      if (key.startsWith(sessionPrefix)) {
        inFlightBySessionMessageKey.delete(key);
      }
    }
  }
