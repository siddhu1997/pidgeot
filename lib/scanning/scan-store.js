...
function sanitizeSourceCheckpoint(checkpoint) {
  return {
    ...
    inFlightMessageCount: checkpoint.inFlightMessageCount,
...
function sanitizeScan(scan) {
  ...
  return {
    ...
    scanId: scan.id,
    senderGroups: listSanitizedSenderGroups(scan.senderGrouping).map((group) => ({
      ...group,
      capturedMessages: buildCapturedMessages(scan, group.id),
    })),
    sourceSummaries: Object.fromEntries(
      SOURCE_SCAN_ORDER
        .filter((source) => scan.sources[source])
        .map((source) => [source, sanitizeSourceCheckpoint(scan.sources[source])]),
    ),
    state: scan.state,
...
export function createScanStore({ now = () => Date.now(), onScanDestroyed = () => {} } = {}) {
  ...
  function setSourceInFlightMessageCount(scanId, { inFlightMessageCount, source }) {
    return updateScan(scanId, (scan) => {
  ...
  function beginSourcePage(scanId, { messageRefs, nextPageToken, source }) {
    return updateScan(scanId, (scan) => {
      ...
      if (!sourceCheckpoint || !SOURCE_SCAN_ORDER.includes(source)) {
        throw new Error(`unknown_scan_source:${source}`);
      }
      ...
      return {
        ...
        counters: {
          ...scan.counters,
          activeMessagesDiscovered: scan.counters.activeMessagesDiscovered + uniqueMessageIds.length,
  ...
  return {
    ...
    setScanFailure,
    setScanLease,
    setSourceInFlightMessageCount,
    transitionScanState,
    updateScan,
  };
}

const SCAN_STORE_KEY = "__pidgeotScanStoreV6i";

export function getScanStore(config) {
  if (globalThis[SCAN_STORE_KEY]) {
    return globalThis[SCAN_STORE_KEY];
  }
  ...
  globalThis[SCAN_STORE_KEY] = store;
