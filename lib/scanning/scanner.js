...
function selectNextSource(scan) {
  for (const source of SOURCE_SCAN_ORDER) {
    const checkpoint = scan.sources[source];

    if (!checkpoint) {
      continue;
    }

    if (checkpoint.pendingMessageIds.length > 0 || !checkpoint.completedAt) {
      return source;
    }
  }
...
function heartbeatActiveScanLease(processingLeaseStore, scan) {
  if (!scan?.leaseId || !scan.sessionId) {
    return null;
  }

  return processingLeaseStore.heartbeat({
    leaseId: scan.leaseId,
    sessionId: scan.sessionId,
  });
}
...
function buildSessionMessageLimit(scan, config, pendingMessageCount = 0) {
  ...
  return {
    ...
    message: isDevelopmentMessageLimitActive(config)
      ? `Showing the first ${maxRetainedMessages} messages for this scan.`
    ...
    const { remainingItems } = await processWithConcurrency(
      ...
      async ({ index, messageId }) => {
        ...
        try {
          ...
          heartbeatActiveScanLease(processingLeaseStore, scan);
  ...
  async function processNextChunk({ session }) {
    ...
    try {
      ...
      if (sourceCheckpoint.pendingMessageIds.length === 0) {
        heartbeatActiveScanLease(processingLeaseStore, activeScan);
    ...
    async startScan({ session }) {
      ...
      const lease = processingLeaseStore.acquireLease({ sessionId: session.id });
       ...
       if (config.devWorkflowSimulationEnabled) {
        workflowSimulationService.resetSession({ sessionId: session.id });
      }
