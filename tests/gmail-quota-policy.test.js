...
describe("gmail quota policy", () => {
  it("exposes only the method costs used by the active Gmail foundation", () => {
    ...
    expect(getGmailMethodQuotaCost("messages.trash")).toBe(5);
  ...
  it("rejects methods that have not been added to the active foundation yet", () => {
    const quotaPolicy = createGmailQuotaPolicy();

    expect(quotaPolicy.canConsume({ methodName: "messages.batchModify", userId: "user-1" })).toMatchObject({
      allowed: false,
